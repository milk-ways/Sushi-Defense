...
public class TowerWeapon : MonoBehaviour
{
    ...
    private WeaponState weaponState = WeaponState.SearchTarget; //타워무기의 상태
    ...
    private IEnumerator SearchTarget()
    {
        while (true)
        {
            ...
            // EnemySpawner의 EnemyList에 현재 맵에 존재하는 모든 적 검사
    ...
    private IEnumerator AttackToTarget()
    {
        while (true)
        {
            // 1. target이 있는지 검사 (다른 발사체에 의한 제거, Goal 지점까지 이동해 삭제 등)
            if (attackTarget == null)
            {
                ChangeState(WeaponState.SearchTarget);
                break;
            ...
            // 2. target이 공격범위 안에 있는지 검사 (공격 범위를 벗어나면 새로운 적 탐색)
